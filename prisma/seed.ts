...
// ) {
//   const doctorPassword = await hash("password", 10);
...
async function main() {
  // Create Admin
  const adminPassword = await hash("100usersByJune", 10);
  const admin = await prisma.user.create({
    data: {
      name: "Admin One",
      email: "ukshiftly@gmail.com",
  ...
  // console.log("Client created:", patient);
